    ...
    <motion.div initial={{ x: -100, opacity: 0 }} whileInView={{ x: 0, opacity: 1 }} transition={{ duration: 1.0 }} viewport={{ once: true }}>
      <div className="py-5">
        <div className="border-bottom">
          <h2 className="text-left text-2xl lg:text-3xl px-4 py-4 font-bold text-underline">Trend Of the Week</h2>
        </div>
        {trenProduct.map((product) => (
          <div key={product.id}>
            <Container className="flex-row lg:flex px-0 pt-4 ">
              <Image src={product.image} className="flex-col-8 w-full md:w-full lg:w-2/5 h-auto max-w-[100%] aspect-[1/1] object-cover rounded-md px-1" />
              <div className="px-4 flex-col-4">
                <h2 onClick={() => onOpenModal(product)} className="font-bold text-2xl lg:text-3xl py-3">
                  {" "}
                  {product.name}
                </h2>
                <p>${product.price}</p>
                <p className="py-2 text-sm xl:text-xl text-color-gray">{product.description}</p>
